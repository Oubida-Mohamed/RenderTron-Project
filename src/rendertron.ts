...
export class Rendertron {
  ...
  async handleRenderRequest(ctx: Koa.Context, url: string) {
    if (!this.renderer) {
      throw new Error('No renderer initialized yet.');
    }
    ...
    try {
      // Ensure the page is fully loaded before serializing the content
      ...
      console.error(error);
      ctx.status = 500;
      ctx.body = 'An error occurred while rendering the page';
    }
  }
  ...
  async handleScreenshotRequest(ctx: Koa.Context, url: string) {
    ...
    try {
      // Ensure the page is fully loaded before taking the screenshot
